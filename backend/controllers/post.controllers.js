...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        const isLiked = post
            .likes
            .some(like => like.user.toString() === currentUserId.toString());

        if (isLiked) {
            //? Unlike the post
            post
                .likes
                .pull({user: currentUserId});
            post.likeCount -= 1;
            ...
            await Notification.deleteOne(
                {from: currentUserId, to: post.user, type: 'like'}
            );
            res
                .status(200)
...
export const commentOnPost = async (req, res) => {
    try {
        ...
        if (!text) 
            return res
                .status(400)
                .json({message: 'Text field is required'});
        const post = await Post.findById(postId);
        if (!post) 
            return res
                .status(404)
        ...
        const notification = new Notification(
            {from: currentUserId, to: post.user, type: 'comment'}
        );
        await notification.save();
